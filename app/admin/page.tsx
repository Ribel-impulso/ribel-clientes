...
export default function AdminPage() {
  ...
  function colorEstado(estado: string, dias: number) {
    if (estado === 'vencida' || dias < 0) return '#c0392b'
  ...
  // Contadores: se basan en el campo "estado" (fuente de verdad, editable a mano),
  // no en la fecha de vencimiento, para que coincidan con lo que se ve en la tabla.
  const totalUsuarios = suscripciones.length
  const totalActivas = suscripciones.filter(s => s.estado === 'activa').length
  const totalVencidas = suscripciones.filter(s => s.estado === 'vencida').length
  const totalEnPrueba = suscripciones.filter(s => s.estado === 'prueba').length
          ...
          <>
            <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', flexWrap: 'wrap' }}>
              <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '16px 24px', flex: 1, minWidth: '150px' }}>
                ...
                <p style={{ margin: 0, fontSize: '28px', fontWeight: 'bold', color: '#161616' }}>{totalUsuarios}</p>
              </div>
              <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '16px 24px', flex: 1, minWidth: '150px' }}>
                <p style={{ margin: 0, color: '#9e9e9e', fontSize: '13px' }}>Activas</p>
                <p style={{ margin: 0, fontSize: '28px', fontWeight: 'bold', color: '#27ae60' }}>{totalActivas}</p>
              </div>
              <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '16px 24px', flex: 1, minWidth: '150px' }}>
                <p style={{ margin: 0, color: '#9e9e9e', fontSize: '13px' }}>Vencidas</p>
                <p style={{ margin: 0, fontSize: '28px', fontWeight: 'bold', color: '#c0392b' }}>{totalVencidas}</p>
              </div>
              <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '16px 24px', flex: 1, minWidth: '150px' }}>
                <p style={{ margin: 0, color: '#9e9e9e', fontSize: '13px' }}>En prueba</p>
                <p style={{ margin: 0, fontSize: '28px', fontWeight: 'bold', color: '#ba9a7d' }}>{totalEnPrueba}</p>
              </div>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {suscripcionesFiltradas.length === 0 ? (
                    <tr>
                      <td colSpan={7} style={{ padding: '20px 16px', textAlign: 'center', color: '#9e9e9e', fontSize: '13px' }}>
                        No se encontraron usuarios con ese email.
                      </td>
                    </tr>
                  ) : (
                    suscripcionesFiltradas.map((s, i) => {
